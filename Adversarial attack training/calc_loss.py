...
import h5py
import os
from model.test_model import eval_loss
...
def calulate_loss_landscape(model, directions):
    ...
    with h5py.File("./3d_surface_test_adv_file.h5", 'r+') as f:
...
def setup_surface_file():
    xmin, xmax, xnum = -1, 1, 100
    ymin, ymax, ynum = -1, 1, 100

    surface_path = "./3d_surface_test_adv_file.h5"
    ...
    with h5py.File(surface_path, 'a') as f:
        print("create new 3d_surface_test_file.h5")
