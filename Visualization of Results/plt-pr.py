...
exampleFile = open('./csv/average_pr_Net.csv')  # 打开csv文件
...
auc_values = ['0.37', '0.75', '0.69', '0.79', '0.56', '0.81', '0.69', '0.74', '0.90']

for i in range(len(models)):
    plt.plot(eval(f'x{i+1}'), eval(f'y{i+1}'), label=f'{models[i]}', lw=lw, color=colors[i])

plt.xlabel("Recall",fontsize=16,family='Times New Roman')
plt.ylabel("Precision",fontsize=16,family='Times New Roman')

plt.xticks(fontsize=12,family='Times New Roman')
plt.yticks(fontsize=12,family='Times New Roman')

handles, labels = plt.gca().get_legend_handles_labels()
labels = [f'{label} (Avg-PR = {auc})' for label, auc in zip(labels, auc_values)]
